...
static class Program
{
    static void Main (string[] args)
    {
        using (var db = new AppContext())
        {
            ...
            #region Миграции
            //Миграция — это изменение схемы БД по определенному плану.
            //То есть это такой скрипт для исполнения в БД, с помощью которого можно актуализировать БД таким образом,
            //чтобы она соответствовала вашим объектам в приложении.
            //Типа если появилось новое свойство, и нужно в БД его отобразить

            //1 Добавляем nuget Microsoft.EntityFrameworkCore.Tools
            //2 Необходимо открыть окно «Консоль диспетчера пакетов» (или Package Manager Console)
            //3 В появившемся окне вы сможете выполнять необходимые команды:

            //1 Первое, что вам понадобится — создать начальную миграцию. Она будет включать в себя процесс создания БД с нуля.
            //2 В консоли команду «Add-Migration Initial», которая создаст миграцию Initial
            //3 Появился ряд новых файлов в папке «Migrations»:
            ...
            //Down - Метод для отката изменений
            //Up - Метод для применения миграции

            //4 Поскольку данная миграция является первичной, она была создана таким образом, чтобы создать базу данных с нуля
            //И если мы попробуем применить данную миграцию с помощью команды Update-Database - ОШИБКА

            //5 Таким образом, начинать работу с миграциями нужно с начала жизни проекта!!!
            //6 Если же у вас уже есть какая-то база и данные в ней, то данную проблему можно решить 2 способами:
            //   а Удалить все таблицы БД и применить миграцию. При этом все данные будут потеряны
            //   б Убрать первую миграцию из папки, оставив файл  ***ModelSnapshot.Затем при изменении модели создать новую миграцию и применить её.

            //Add-Migration Initial - создание начальной миграции
            //Add-Migration [Имя] - создание миграции
            //Update-Database - применение к БД
